...
def _build_covert_personality(day_stem, day_master_traits, pillars, ten_gods_analysis, ten_gods_traits):
    ...
    for pillar_key in ['year_pillar', 'hour_pillar']:  # 年柱和时柱代表内在
        data = ten_gods_analysis.get(pillar_key, {})
        for hd in data.get('branch_hidden', []):
            tg = hd.get('ten_god', '')
            if tg in ['正印', '偏印'] and tg in ten_gods_traits:
                covert_notes.append(f"有{tg}入命，内心渴望被认可与保护，有深厚的学习欲望")
                break
            elif tg in ['正财', '偏财'] and tg in ten_gods_traits:
                covert_notes.append(f"有{tg}入命，内心深处重视物质安全感和家庭保障")
                break
