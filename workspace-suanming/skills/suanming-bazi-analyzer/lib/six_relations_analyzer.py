...
def _analyze_children(pillars, ten_gods_analysis, yong_shen_info, gender):
    ...
    for pillar_key, data in ten_gods_analysis.items():
        stem_tg = data.get('stem_ten_god', '')
        if stem_tg in child_stars:
            child_count += 1
        for hd in data.get('branch_hidden', []):
            if hd.get('ten_god') in child_stars:
                child_count += 1

    result = {'child_star_count': child_count, 'child_label': child_label}

    if child_count == 0:
        result['children_bond'] = '子女缘较淡，或子女较少，晚年需自立'
    elif child_count == 1:
        result['children_bond'] = '子女缘一般，通常有一子女，感情平稳'
    elif child_count >= 2:
        result['children_bond'] = '子女星旺，子女贤孝，晚年有子女福，家庭热闹'

    # 时柱代表晚年子女
    hour_tg = ten_gods_analysis.get('hour_pillar', {}).get('stem_ten_god', '')
    if hour_tg in child_stars:
        result['late_life'] = '时柱现子女星，晚年享子女福，晚景温馨'
    elif hour_tg in ['正印', '偏印']:
        result['late_life'] = '时柱见印星，晚年多有贵人助力，精神富足，适合修身养性'
    elif hour_tg in ['正财', '偏财']:
