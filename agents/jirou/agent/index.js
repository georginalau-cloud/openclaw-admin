...
/**
 * index.js - 肌肉喵 Agent 入口点
 *
 * 职责：
 *   1. 加载环境变量
 *   2. 初始化 agent 环境（目录结构等）
 *   3. 启动消息监听（stdin 或 HTTP）
  ...
  process.stdin.on('data', async (chunk) => {
    ...
    for (const line of lines) {
      ...
      try {
        ...
        logger.warn('[index] 无法解析的消息行，跳过:', trimmed.slice(0, 100));
      }
    }
  });
  ...
  process.stdin.on('end', () => {
    ...
    if (buffer.trim()) {
      try {
        ...
        logger.warn('[index] 无法解析最后的消息，跳过');
      }
    }
...
// ---------------------------------------------------------------------------
async function main() {
  logger.info('[index] 肌肉喵 Agent 启动中...');
  ...
  try {
    ...
    startStdinListener();
    logger.info('[index] 肌肉喵 Agent 已就绪 💪');
  } catch (err) {
